require([
    ...
    //  Graphics Layer to display AQI markers
    var graphicsLayer = new GraphicsLayer();
    myMap.add(graphicsLayer);

    // 20 markers(latitude, longitude)
    var districts = [
    ...
    // Function to fetch AQI data for each district
    function fetchAQIData(district) {
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                if (data.status === "ok") {
                    var aqi = data.data.aqi; // Get AQI value
                    var mainPollutant = data.data.dominentpol; // Get dominant pollutant
                    var markerColor = getAQIColor(aqi); // Determine marker color
                    var marker = {
                        type: "simple-marker",
                        style: "circle",
                        color: markerColor,
...
//color
    ...
    var locate = new Locate({
        ...
        goToOverride: function(view, options) {
            options.target.scale = 1500; // Zoom when locating
